...
const ProductDetail = () => {
    ...
    const { data: productById, isLoading, isError } = useQuery({
        queryKey: ['products', productDetailId], 
        queryFn: () => getProductById(productDetailId)
    });

    if (isLoading) return <p>Chargement...</p>;
    if (isError) return <p>Une erreur est survenue lors de la récupération du produit.</p>;
    ...
    return (
        <>
            ...
            {productById && <ProductDetailCard key={productById.id} productDetail={productById} /> }
            <Comment />
            <Footer />
        </>
    );
