...
// Accessibility component to provide UI accessibility options
const Accessibility = () => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [dyslexicFont, setDyslexicFont] = useState(false);
  const [lineSpacing, setLineSpacing] = useState(false);

  // Toggle the accessibility options modal
  const toggleModal = () => {
    setIsModalOpen(!isModalOpen);
  };

  // Change the base text size of the document
  const changeTextSize = (size) => {
    document.documentElement.style.fontSize = size;
  };

  // Toggle readable font style
  const changeFont = () => {
    setDyslexicFont(!dyslexicFont);
    document.body.classList.toggle("dyslexic-font");
  };

  // Toggle increased line spacing
        ...
        <div className="accessibility">
          ...
          >
            ...
            <p id="modal-description">
              Select the accessibility options to adjust as per your needs.
            </p>

            <div className="option-group">
              <label>Text Size:</label>
              <div className="btn-group">
                <button onClick={() => changeTextSize("1.2rem")}>
                  <MdTextIncrease />
                </button>
                <button onClick={() => changeTextSize("1rem")}>Normal</button>
                <button onClick={() => changeTextSize("0.8rem")}>
                  <MdTextDecrease />
                </button>
              </div>
            </div>
            ...
            <div className="option-group">
              ...
              <button onClick={changeLineSpacing}>
                {lineSpacing
                  ? "Disable increased line spacing"
